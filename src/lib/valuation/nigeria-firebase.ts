...
export async function estimateNigerianPropertyValue(
  ...
  let estimatedValue = sizeValue + bedroomValue + ageAdjustment + amenityValue;
  if (marketValue > 0) {
      // Average the calculated value with the market-derived value for a blended result
      estimatedValue = (estimatedValue + marketValue) / 2;
  }
...
function calculateFromComparables(comparables: any[], input: NigerianPropertyValuationInput) {
  ...
  const validComparables = comparables.filter(c => c.price && c.sqft > 0)
  ...
  const totalValue = validComparables.reduce((sum, comp) => {
    const pricePerSqm = comp.price / (comp.sqft / 10.764) // comp.sqft is in sqft, need to convert to sqm for comparison
...
function getMarketTrend(comparables: any[]): string {
  ...
  const sortedByDate = comparables.sort((a,b) => a.createdAt.toDate() - b.createdAt.toDate());
  ...
  if(olderAvg === 0) return 'Insufficient data for trend analysis';
